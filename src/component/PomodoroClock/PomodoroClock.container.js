...
export class PomodoroClockContainer extends PureComponent {
  ...
  containerProps() {
    const { breakLength, sessionLength, sessionTime, sessionLabel } =
      this.state;
  ...
  _timerCountDown() {
    const { timeInSeconds, isBreak, breakLength, sessionLength, audio } =
      this.state;
  ...
  render() {
    return (
      <PomodoroClock {...this.containerFunctions} {...this.containerProps()} />
    );
  }
}
